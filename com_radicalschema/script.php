	...
	public function register(Container $container)
	{
		$container->set(InstallerScriptInterface::class, new class ($container->get(AdministratorApplication::class)) implements InstallerScriptInterface {
			...
			protected function checkCompatible(): bool
			{
				...
				{
					$app->enqueueMessage(Text::sprintf('PKG_RADICALSCHEMA_ERROR_COMPATIBLE_JOOMLA', $this->minimumJoomla),
						'error');
				...
				{
					$app->enqueueMessage(Text::sprintf('PKG_RADICALSCHEMA_ERROR_COMPATIBLE_PHP', $this->minimumPhp),
						'error');
			...
			/**
			 * Method to remove default radicalschema plugin.
			 *
			 * @return  void
			 *
			 * @since  __DEPLOY_VERSION__
			 */
			protected function removeDefaultPlugin(): void
			{
				...
				$query     = $this->db
					...
					->where($this->db->quoteName('folder') . ' = ' . $this->db->quote('radicalschema'))
					->where($this->db->quoteName('element') . ' = ' . $this->db->quote('default'));
				$row       = $this->db->setQuery($query)->loadObject();

				if ($row)
				{
					$installer->uninstall($row->type, $row->extension_id);
				}
			}
		});
	}
