    ...
    public function register(Container $container)
    {
        $container->set(InstallerScriptInterface::class, new class ($container->get(AdministratorApplication::class)) implements InstallerScriptInterface {
            ...
            protected function checkCompatible(): bool
            {
                ...
                {
                    $app->enqueueMessage(Text::sprintf('PLG_RADICALSCHEMA_IMAGE_ERROR_COMPATIBLE_JOOMLA', $this->minimumJoomla),
                        'error');
                ...
                {
                    $app->enqueueMessage(Text::sprintf('PLG_RADICALSCHEMA_IMAGE_ERROR_COMPATIBLE_PHP', $this->minimumPhp),
                        'error');
            ...
            protected function generateKey()
            {
				...
				$params = $db->setQuery(
					$db->getQuery(true)
						...
						->where($db->quoteName('element') . ' = ' . $db->quote('com_radicalschema'))
				)->loadResult();

				$params = new Registry($params);

				// Set secret key
				if (!$params->get('image_imagetype_generate_secret_key'))
				{
					$params['image_imagetype_generate_secret_key'] = uniqid(rand());

					// Prepare component object
					$data          = new stdClass();
					$data->type    = 'component';
					$data->element = 'com_radicalschema';
					$data->params  = $params->toString();

					// Update
					$db->updateObject('#__extensions', $data, array('type', 'element'));
				}
            }
        });
    }
